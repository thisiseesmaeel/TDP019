...
class Dice_node
  ...
  def eval()
    ...
    if @a == 1
      ...
      (1..@a.eval).inject(0) {|sum, _| sum + rand(@b.eval) + 1 }
    end
  end
end
...
class DiceRoller
  ...
  def initialize
    @diceParser = Parser.new("dice roller") do
      ...
      start :expr do 
        match(:expr, '+', :term) {|a, _, b| Addition_node.new(a, b) }
        match(:expr, '-', :term) {|a, _, b| Subtraction_node.new(a, b) }
        match(:term)
      end
      
      rule :term do 
        match(:term, '*', :dice) {|a, _, b| Multiplication_node.new(a, b) }
        match(:term, '/', :dice) {|a, _, b| Division_node.new(a, b) }
        match(:dice)
      end

      rule :dice do
        match(:atom, 'd', :sides) {|a, _, b| Dice_node.new(a, b) }
        match('d', :sides) {|_, b| Dice_node.new(1, b) }
        match(:atom)
      end
      ...
      rule :atom do
        ...
        match('(', :expr, ')') {|_, a, _| Expression_node.new(a) }
      end
    end
  end
  ...
  def roll
    ...
    if done(str) then
      ...
      root_node = @diceParser.parse str
      puts "=> #{root_node.eval}"
      roll
    end
  end
