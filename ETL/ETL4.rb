...
class Etl
    ...
    def initialize
      ...
      #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+- BEGIN BNF +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ 
        ...
        start :program do
            match("startprogram", :statement, "endprogram") { |_, states, _| states }
            end
...
=end
       ...
       rule :bool_logic do
        ...
        rule :bool_expr do
            match(:bool_word) { |bool_exp| bool_exp}
            match(:bool_symbol) { |bool_exp| bool_exp }
            ##match(:id) { |var| var }
        ...
        rule :bool_word do
            match(:expr, 'less', 'than', :expr) { |expr1, _, _, expr2| Expr.new('less than', expr1, expr2) }
            match(:expr, 'greater', 'than', :expr) { |expr1, _, _, expr2| Expr.new('greater than', expr1, expr2) }
            match(:expr, 'less', 'than', 'or', 'equal', 'to', :expr) { |expr1, _, _, _, _, _, expr2| Expr.new('less than or equal to', expr1, expr2) }
            match(:expr, 'greater', 'than', 'or', 'equal', 'to', :expr) { |expr1, _, _, _, _, _, expr2| Expr.new('greater than or equal to', expr1, expr2) }
            match(:expr, 'not', 'equal', 'to', :expr) { |expr1, _, _, _, expr2| Expr.new('not equal to', expr1, expr2) }
        ...
        rule :atom do
            ...
            match("-", Float) {​​​​​ |a, b| Constant_negativ.new(a, b) } ​​​​​
            match("-", Integer) {​​​​ ​|a, b| Constant_negativ.new(a, b) }​​​​​

            ##match(True) { |true| Cons 
            ##match(:id) { |id| id }
            match('(',:expr,')') { |_,expr,_| Expression_node.new(expr) }
            ##match(:logic) { |logic| logic}
            ##match(:function_call)
            end
